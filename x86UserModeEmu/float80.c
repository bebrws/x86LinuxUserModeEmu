// From ish, word for word, byte for byte
...
// bit_return from https://stackoverflow.com/questions/1697425/how-to-print-out-each-bit-of-a-floating-point-number
// a is the current number we are checking bits on
// loc is the bit to check
// returns 1 or 0 depending on if bit at location log in a number a is on or off
...
// print_struct_binary
// s_to_print is a void * to any type
// len is the number of bytes that we can safely print from the pointer, aka the size of the type
// new_line is whether or not a new line is printed after printing out the binary representation
void print_struct_binary(void * s_to_print, int len, bool new_line)
{
    uint8_t *to_print = s_to_print;
    for (int b = len-1; b >= 0; b--) {
        for (int i = 7; i >= 0; i--) {
            printf("%d", bit_return(to_print[b], i));
        }
    }
    if (new_line) {
        printf("\n");
...
void debug_print_bbfloat80(float80 f) {
    printf("bbfloat80 in binary:\n");
    print_struct_binary((void *)&f, 10, true);
    
    printf("f.signif: %llu  -- in binary == ", f.signif);
    print_struct_binary((void *)&f.signif, 8, true);
    
    printf("f.exp: %d  -- in binary == ", f.exp);
